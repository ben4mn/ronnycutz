        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '12px', maxWidth: '500px', marginBottom: '24px' }}>
          {days.filter(([d]) => !['sat', 'sun'].includes(d)).map(([day, slots], i) => (
            <motion.div
              ...
              style={{ background: slots.length ? '#fff' : '#f5f5f5', border: '2.5px solid #111', borderRadius: '12px', padding: '16px 20px', boxShadow: '3px 3px 0 #111', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}
            >
