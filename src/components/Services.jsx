...
export function SectionHeader({ eyebrow, title, blue }) {
  return (
    <div className="mb-8">
      ...
      <h2 style={{ fontSize: '28px', fontWeight: 900, marginTop: '12px', color: '#111' }}>{title}</h2>
    </div>
  );
}
...
export default function Services() {
  return (
    <section id="services" style={{ padding: '36px 16px', background: '#fff', borderBottom: '3px solid #111' }}>
      <style>{`
          ...
          {services.map((s, i) => (
            <motion.a
              ...
              style={{
                ...
                padding: '18px', boxShadow: '4px 4px 0 #111', cursor: 'pointer',
              }}
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '6px' }}>
                <h3 style={{ fontSize: '15px', fontWeight: 800, color: '#111', margin: 0, paddingRight: '8px' }}>{s.name}</h3>
                <span style={{ fontSize: '22px', fontWeight: 900, color: '#E03A2F', flexShrink: 0 }}>${s.price}</span>
              </div>
