...
router.get('/config', requireToken, (req, res) => {
  ...
  res.json({
    calendar_feed_url_https: httpsUrl,
    calendar_feed_url_webcal: webcalUrl,
    feed_configured: Boolean(feedToken),
  });
});
...
router.get('/bookings', requireToken, (req, res) => {
  const rows = db
    .prepare(
      `SELECT id, service_name, service_price, start_iso, duration_min,
              client_name, client_phone, client_email, notes, status, created_at
       FROM bookings
       WHERE start_iso >= datetime('now', '-7 days')
...
router.post('/blocks', requireToken, (req, res) => {
  ...
  const result = db
    .prepare('INSERT INTO blocked_slots (start_iso, end_iso, reason) VALUES (?, ?, ?)')
    .run(start_iso, end_iso, reason || null);
