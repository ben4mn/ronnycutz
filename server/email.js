...
export function getTransporter() {
  ...
  if (!process.env.GMAIL_USER || !process.env.GMAIL_APP_PASSWORD) {
    console.warn('[email] GMAIL_USER or GMAIL_APP_PASSWORD not set â emails disabled');
    return null;
  }
...
export async function sendBookingEmails(booking, baseUrl) {
  ...
  const clientHtml = `
    <div style="${baseStyle()}">
      <h1 style="color:#4A7FD4;margin:0 0 16px;font-size:28px;font-weight:900;">RonnyCutz â</h1>
      ...
      <p style="color:#666;font-size:14px;">You'll get another email once it's confirmed. â RonnyCutz</p>
    </div>`;
  ...
  const ownerHtml = `
    <div style="${baseStyle()}">
      ...
      <div style="background:#fff;border:2px solid #111;padding:16px 20px;margin:20px 0;border-radius:8px;">
        <p style="margin:4px 0;font-size:16px;font-weight:800;">${booking.service_name} â ${when}</p>
  ...
  try {
    await t.sendMail({ from: process.env.GMAIL_USER, to: booking.client_email, subject: 'RonnyCutz â Booking Request Received', html: clientHtml });
    if (process.env.NOTIFY_EMAIL) {
      await t.sendMail({ from: process.env.GMAIL_USER, to: process.env.NOTIFY_EMAIL, subject: '[RonnyCutz] New booking â ' + booking.client_name + ' @ ' + when, html: ownerHtml });
    }
...
export async function sendApprovalEmail(booking, baseUrl) {
  ...
  const html = `
    <div style="${baseStyle()}">
      <h1 style="color:#4A7FD4;margin:0 0 16px;font-size:28px;font-weight:900;">You're Confirmed! â</h1>
      ...
      <div style="background:#EEF4FF;border:2px solid #4A7FD4;border-radius:8px;padding:16px 20px;margin:20px 0;">
        <p style="margin:0;font-size:15px;font-weight:800;color:#111;">ð When you arrive:</p>
      ...
      <p style="color:#666;font-size:12px;margin-top:24px;">â RonnyCutz Â· 6522 84th St, Lubbock TX</p>
    </div>`;

  await t.sendMail({
    from: process.env.GMAIL_USER,
    to: booking.client_email,
    subject: 'RonnyCutz â Appointment Confirmed! ' + when,
    html,
...
export async function sendDenialEmail(booking, baseUrl) {
  ...
  await t.sendMail({ from: process.env.GMAIL_USER, to: booking.client_email, subject: 'RonnyCutz â Booking Update', html });
}