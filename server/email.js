...
export function getTransporter() {
  ...
  if (!process.env.GMAIL_USER || !process.env.GMAIL_APP_PASSWORD) {
    console.warn('[email] GMAIL_USER or GMAIL_APP_PASSWORD not set ÃÂ¢ÃÂÃÂ emails disabled');
    return null;
  }
...
export async function sendBookingEmails(booking, baseUrl) {
  ...
  const clientHtml = `
    <div style="${baseStyle()}">
      <h1 style="color:#4A7FD4;margin:0 0 16px;font-size:28px;font-weight:900;">RonnyCutz ÃÂ¢ÃÂÃÂ</h1>
      ...
      <p style="color:#666;font-size:14px;">You'll get another email once it's confirmed. ÃÂ¢ÃÂÃÂ RonnyCutz</p>
    </div>`;
  ...
  const ownerHtml = `
    <div style="${baseStyle()}">
      ...
      <div style="background:#fff;border:2px solid #111;padding:16px 20px;margin:20px 0;border-radius:8px;">
        <p style="margin:4px 0;font-size:16px;font-weight:800;">${booking.service_name} ÃÂ¢ÃÂÃÂ ${when}</p>
      ...
      <a href="${adminUrl}" style="display:inline-block;background:#4A7FD4;color:#fff;padding:12px 28px;border-radius:50px;font-weight:800;font-size:15px;text-decoration:none;border:2.5px solid #111;box-shadow:3px 3px 0 #111;">
        Open Admin Page ÃÂ¢ÃÂÃÂ
      </a>
    </div>`;

  try {
    await t.sendMail({ from: process.env.GMAIL_USER, to: booking.client_email, subject: 'RonnyCutz ÃÂ¢ÃÂÃÂ Booking Request Received', html: clientHtml });
    const notifyEmail = process.env.NOTIFY_EMAIL || 'aaron.moreno1024@gmail.com';
    if (notifyEmail) {
      await t.sendMail({ from: process.env.GMAIL_USER, to: notifyEmail, subject: '[RonnyCutz] New booking ÃÂ¢ÃÂÃÂ ' + booking.client_name + ' @ ' + when, html: ownerHtml });
    }
...
export async function sendApprovalEmail(booking, baseUrl) {
  ...
  const html = `
    <div style="${baseStyle()}">
      <h1 style="color:#4A7FD4;margin:0 0 16px;font-size:28px;font-weight:900;">You're Confirmed! ÃÂ¢ÃÂÃÂ</h1>
      ...
      <div style="background:#EEF4FF;border:2px solid #4A7FD4;border-radius:8px;padding:16px 20px;margin:20px 0;">
        <p style="margin:0;font-size:15px;font-weight:800;color:#111;">ÃÂ°ÃÂÃÂÃÂ When you arrive:</p>
      ...
      <p style="color:#666;font-size:12px;margin-top:24px;">ÃÂ¢ÃÂÃÂ RonnyCutz ÃÂÃÂ· 6522 84th St, Lubbock TX</p>
    </div>`;

  await t.sendMail({
    from: process.env.GMAIL_USER,
    to: booking.client_email,
    subject: 'RonnyCutz ÃÂ¢ÃÂÃÂ Appointment Confirmed! ' + when,
    html,
  ...
  const html = `
    <div style="${baseStyle()}">
      ...
      <a href="${baseUrl}/#book" style="display:inline-block;background:#E03A2F;color:#fff;padding:12px 28px;border-radius:50px;font-weight:800;font-size:14px;text-decoration:none;border:2.5px solid #111;box-shadow:3px 3px 0 #111;margin-top:8px;">
        Book Again ÃÂ¢ÃÂÃÂ
      </a>
      <p style="color:#666;font-size:12px;margin-top:24px;">ÃÂ¢ÃÂÃÂ RonnyCutz ÃÂÃÂ· 6522 84th St, Lubbock TX</p>
    </div>`;

  await t.sendMail({ from: process.env.GMAIL_USER, to: booking.client_email, subject: 'RonnyCutz ÃÂ¢ÃÂÃÂ Booking Update', html });
}